...
export function CourseAIAssistant({ courseId }: CourseAIAssistantProps) {
  ...
  const configureApiKey = () => {
    const apiKey = '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';
        ...
        for (const material of fileMaterials) {
          ...
          try {
            if (material.fileUrl) {
              ...
              const file = new File([blob], material.name, { type: material.mimeType || 'application/octet-stream' });
            ...
            console.error(`Error extracting content from ${material.name}:`, error);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const errorMessage: ChatMessage = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: 'Lo siento, hubo un error al procesar tu pregunta. Verifica que tu API key de Groq esté configurada correctamente.',
        timestamp: new Date()
      };
  ...
  const getAvailableNotesCount = () => {
    return includeNotes ? courseNotes.length + courseGrades.length + courseEvents.length : 0;
  };
      ...
      <div className="p-4 border-b border-dark-border bg-dark-bg-secondary">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-dark-text-muted">
                {getAvailableMaterialsCount()} materiales • {getAvailableNotesCount()} notas
              </p>
            </div>
          </div>
        ...
        {showSettings && (
          <div className="mt-4 p-4 bg-dark-bg-primary rounded-lg border border-dark-border">
            <h3 className="text-sm font-semibold text-dark-text-primary mb-3">Configuración del Asistente</h3>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 ? (
          <div className="text-center text-dark-text-muted py-8">
            <Bot size={48} className="mx-auto mb-4 text-course-blue opacity-50" />
            <h3 className="text-lg font-semibold text-dark-text-secondary mb-2">
              Asistente IA del Curso
            </h3>
            <p className="text-sm max-w-md mx-auto">
              Haz preguntas sobre los materiales, notas y contenido de {course?.name}. 
              El asistente solo responderá basándose en la información del curso.
            </p>
            {getAvailableMaterialsCount() === 0 && (
              <p className="text-sm text-yellow-400 mt-3">
                💡 Sube algunos archivos al curso para obtener mejores respuestas
              </p>
            )}
          </div>
        ) : (
      ...
      <div className="p-4 border-t border-dark-border bg-dark-bg-secondary">
        ...
        {getAvailableMaterialsCount() > 0 && (
          <p className="text-xs text-dark-text-muted mt-2">
            Usando {getAvailableMaterialsCount()} archivos y {getAvailableNotesCount()} notas como contexto
          </p>
        )}
      </div>
    </div>

...
export interface Note {
  id: string;
  courseId: string;
  type: 'parcial' | 'final' | 'practica' | 'tarea' | 'proyecto' | 'participacion';
  description: string;
  value: number; // 0-20
...
export const dbUtils = {
  ...
  async clearAllData() {
    await db.transaction('rw', [db.courses, db.files, db.fileTexts, db.ragChunks, 
                         db.notes, db.events, db.todos, db.quickNotes, db.chatMessages], 
                         async () => {
  ...
  async importData(data: any) {
    await db.transaction('rw', [db.courses, db.notes, db.events, db.todos, db.quickNotes], async () => {
      if (data.courses) {
  ...
  async getStats() {
    const [coursesCount, filesCount, notesCount, eventsCount, todosCount] = await Promise.all([

...
import { db, initDatabase, type Course, type FileRecord, type Note, type Event, type Todo, type QuickNote } from '../db/database';
import { storage, type AppSettings, type UserPreferences } from '../storage/localStorage';
...
interface AppActions {
  ...
  // Cursos
  addCourse: (courseData: Omit<Course, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>;
  ...
  updateFileTags: (courseId: string, fileId: string, tags: string[]) => Promise<void>;
  ...
  updateCourseGrade: (courseId: string, gradeId: string, updates: {
    name?: string;
    type?: 'exam' | 'quiz' | 'project' | 'homework' | 'participation' | 'other';
    weight?: number;
    score?: number;
  ...
  updateCourseEvent: (courseId: string, eventId: string, updates: {
    title?: string;
    description?: string;
    date?: Date;
    time?: string;
  ...
  // Notas
  addNote: (courseId: string, noteData: Omit<Note, 'id' | 'courseId' | 'createdAt' | 'updatedAt'>) => Promise<void>;
  updateNote: (courseId: string, noteId: string, updates: Partial<Note>) => Promise<void>;
  deleteNote: (courseId: string, noteId: string) => Promise<void>;
  
  // Eventos
  addEvent: (eventData: Omit<Event, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>;
  updateEvent: (eventId: string, updates: Partial<Event>) => Promise<void>;
  deleteEvent: (eventId: string) => Promise<void>;
  
  // Todos
  addTodo: (courseId: string, todoData: Omit<Todo, 'id' | 'courseId' | 'createdAt' | 'updatedAt'>) => Promise<void>;
  updateTodo: (courseId: string, todoId: string, updates: Partial<Todo>) => Promise<void>;
  deleteTodo: (courseId: string, todoId: string) => Promise<void>;
  toggleTodo: (courseId: string, todoId: string) => Promise<void>;
  
  // Notas rápidas
  addQuickNote: (noteData: Omit<QuickNote, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>;
  updateQuickNote: (noteId: string, updates: Partial<QuickNote>) => Promise<void>;
...
export const useAppStore = create<AppState & AppActions>()(
  subscribeWithSelector((set, get) => ({
      ...
      try {
        ...
        try {
          ...
          for (const course of (initialState.courses || [])) {
            try {
              const [courseFiles, courseNotes, courseTodos] = await Promise.allSettled([
              ...
              console.warn(`⚠️ Error cargando datos del curso ${course.id}:`, courseError);
              (initialState.files || {})[course.id] = [];
              (initialState.notes || {})[course.id] = [];
              (initialState.todos || {})[course.id] = [];
            }
          ...
          console.warn('⚠️ Error cargando datos de la base de datos, usando estado vacío:', dbError);
        }
    ...
    deleteCourse: async (id) => {
      ...
      await db.transaction('rw', [db.courses, db.files, db.notes, db.events, db.todos], async () => {
    ...
    deleteFile: async (courseId, fileId) => {
      ...
      set(state => ({
        files: {
          ...state.files,
          [courseId]: state.files[courseId]?.filter(file => file.id !== fileId) || []
        }
      }));
    },
    ...
    updateFileTags: async (courseId, fileId, tags) => {
      ...
      set(state => ({
        files: {
          ...state.files,
          [courseId]: state.files[courseId]?.map(file =>
            file.id === fileId ? { ...file, tags, updatedAt: new Date() } : file
          ) || []
        }
      }));
    },
      ...
      set(state => ({
        files: {
          ...
          [courseId]: state.files[courseId]?.map(material =>
            material.id === materialId ? { ...material, ...updates, updatedAt: new Date() } : material
          ) || []
        }
      }));
    },
    ...
    moveCourseMaterial: async (courseId, materialId, newParentId) => {
      ...
      set(state => ({
        files: {
          ...state.files,
          [courseId]: state.files[courseId]?.map(material =>
            material.id === materialId ? { ...material, parentId: newParentId, updatedAt: new Date() } : material
          ) || []
        }
      }));
    },
    ...
    updateCourseGrade: async (courseId, gradeId, updates) => {
      ...
      set((state: any) => ({
        grades: {
          ...state.grades,
          [courseId]: state.grades[courseId]?.map((grade: any) =>
            grade.id === gradeId ? { ...grade, ...updates, updatedAt: new Date() } : grade
          ) || []
        }
      }));
    },
    ...
    deleteCourseGrade: async (courseId, gradeId) => {
      ...
      set((state: any) => ({
        grades: {
          ...state.grades,
          [courseId]: state.grades[courseId]?.filter((grade: any) => grade.id !== gradeId) || []
        }
      }));
    },
    ...
    updateCourseEvent: async (courseId, eventId, updates) => {
      ...
      set((state: any) => ({
        courseEvents: {
          ...state.courseEvents,
          [courseId]: state.courseEvents[courseId]?.map((event: any) =>
    ...
    deleteCourseEvent: async (courseId, eventId) => {
      ...
      set((state: any) => ({
        courseEvents: {
          ...state.courseEvents,
          [courseId]: state.courseEvents[courseId]?.filter((event: any) => event.id !== eventId) || []
        }
      }));
    },
    ...
    updateNote: async (courseId, noteId, updates) => {
      ...
      set(state => ({
        notes: {
          ...state.notes,
          [courseId]: state.notes[courseId]?.map(note =>
    ...
    deleteNote: async (courseId, noteId) => {
      ...
      set(state => ({
        notes: {
          ...state.notes,
          [courseId]: state.notes[courseId]?.filter(note => note.id !== noteId) || []
        }
      }));
    },
    ...
    updateTodo: async (courseId, todoId, updates) => {
      ...
      set(state => ({
        todos: {
          ...state.todos,
          [courseId]: state.todos[courseId]?.map(todo =>
            todo.id === todoId ? { ...todo, ...updates, updatedAt: new Date() } : todo
          ) || []
        }
      }));
    },
    ...
    deleteTodo: async (courseId, todoId) => {
      ...
      set(state => ({
        todos: {
          ...state.todos,
          [courseId]: state.todos[courseId]?.filter(todo => todo.id !== todoId) || []
        }
      }));
    },
    ...
    loadCourseData: async (courseId) => {
      ...
      const state = get();
      if (!state.files[courseId] || !state.notes[courseId] || !state.todos[courseId]) {
